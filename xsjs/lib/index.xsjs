...
body += "<li>Continuing as the SYSTEM user, Create a new role called <strong>iot_glue</strong> and grant to it the role <strong>mta_db_only.db.roles::iot_admin</strong>.</li>\n";
body += "<li>Grant the DB user the <strong>sap.hana.xs.ide.roles::CatalogDeveloper</strong> role and the <strong>iot_glue</strong> role. Note: You can't grant the mta_db_only.db.roles::iot_admin role directly.</li>\n";
...
body += "<li>Expand the Calalog and the mta_db_only_HDI_CONTAINER_1 schema, notice that table <strong>mta_db_only.db.data::sensors.temp</strong> is available.</li>\n";
body +="</ul>\n";
body += "<br />\n";
body += "</body>\n";
body += "</html>\n";
